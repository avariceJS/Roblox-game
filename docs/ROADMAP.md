...
## Phase 3 — Dispatch (отправка монстра) ← **СЛЕДУЮЩИЙ**

- [ ] UI: выбор монстра + выбор цели (база игрока / NPC-дом)
- [ ] Монстр **идёт сам** к цели (path или teleport с задержкой — на усмотрение реализации)
- [ ] Способность на месте (Slime → липкая лужа на целевой базе)
- [ ] Награда (монеты, chaos), fatigue после задания
- [ ] Состояния OnMission → Idle / Fatigued

## Phase 4 — PvP Attack

- [ ] Список баз других игроков на сервере
