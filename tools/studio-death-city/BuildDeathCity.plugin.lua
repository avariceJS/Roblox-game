local toolbar = plugin:CreateToolbar("Rent-a-Monster")
local button = toolbar:CreateButton(
	"Death City",
	"Rebuild city: invisible spawns, houses marked",
	"rbxassetid://6034287594"
)
button.ClickableWhenViewportHidden = true

local function buildDeathCity()
...
button.Click:Connect(function()
	local ok, err = pcall(buildDeathCity)
	if ok then print("[DeathCity] plugin OK") else warn("[DeathCity] ERROR:", err) end
end)

print("[DeathCity] plugin loaded")
